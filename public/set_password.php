...
<div class="login-layout">
  <div class="login-left">
    <div class="login-logo-wrap">
      <img src="/website/ticketing-system/public/assets/bugatti-logo.png" alt="Company logo" class="login-logo" onerror="this.style.display='none'" />
    </div>
    <h2 class="login-title">Set Your New Password</h2>
    <p class="login-subtitle">Hello, <?= $userName ?>. For your account security please choose a new password. It must be at least 4 characters and must not match your previous password.</p>
  ...
  <div class="container">
    <form id="setPwdForm">
      <div class="field">
        ...
        <input id="pwd1" name="password" type="password" required minlength="4" />
      </div>
      <div class="field">
        <label for="pwd2">Confirm new password</label>
        <input id="pwd2" name="password_confirm" type="password" required minlength="4" />
      </div>
...
document.getElementById('setPwdForm').addEventListener('submit', async function(e){
  ...
  if (p1.length < 4) { msgEl.textContent = 'Password must be at least 4 characters'; return; }
